...
## 支持以下数据类型
* basic (string,number,bool),basic&int,basic&float,basic&double
* boolean  布尔
* string 字符串
* date 日期类型
...
## 表头规则
* basic基本数据类型(string,int,float,double,bool)时候，一般不需要设置会自动判断，但是也可以明确声明数据类型。
...
* 对象：此列表头的命名形式 `列名#{}` 。
* 对象数组：此列表头的命名形式`列名#[{}]` 。
* json字符串：此列表头的命名形式`列名#json` 。

##特殊对象输出
请在第1行第1列关键字$object或$OBJECT
