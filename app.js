...
function exportJson(args) {
    ...
    var logger = path.join(__dirname, config.xlsx.log);

    utils.mkSubDir(__dirname, logger);

    logger = path.resolve(logger, ymd + ' ' + hms + '.txt');
    if (typeof args === 'undefined' || args.length === 0) {
        glob(config.xlsx.src, function(err, files) {
            if (err) {
                console.error("exportJson error:%j", err);
                throw err;
            }

            //console.log('files: %j', files);
            files.forEach(function(element, index, array) {
                xlsx.toJson(logger, path.join(__dirname, element), path.join(__dirname, config.xlsx.dest));
            });
        })
    } else {
